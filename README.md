...
## Описание задачи
Цель этой задачи - определить среднее количество заказов в месяц, которые не доставляются по разным причинам.
...
## Описание задачи
Цель этой задачи - определить день недели, в который каждый товар чаще всего покупается.
...
## Результаты
В результате анализа было определено, в какой день недели каждый товар чаще всего покупается. Ответ представлен в таблице выше, где для каждого товара указан день недели, в который он чаще всего покупается.

# Задача 4: Определение среднего числа покупок в неделю для каждого пользователя (по месяцам)

## Описание задачи
Цель этой задачи - определить, сколько в среднем покупок пользователь делает в неделю, а также учитывать количество недель в месяце.
